...
public class IdleLayer implements IMotionLayer {
    ...
    public MotionState processMotion(UUID characterId, MotionState currentState, float deltaTime) {
        ...
        float breathingOffset = (float) Math.sin(System.currentTimeMillis() / 1000.0)
                * BREATHING_AMPLITUDE;
    ...
    public boolean validateMotionState(MotionState motionState) {
        // Im Idle-Layer sollte die Geschwindigkeit immer 0 sein
    ...
    public MotionState interpolateStates(MotionState start, MotionState end, float factor) {
        ...
        Position interpolatedPos = interpolatePosition(start.getPosition(),
                ...
                factor);
        Rotation interpolatedRot = interpolateRotation(start.getRotation(),
                end.getRotation(),
                factor);
    ...
    public void reset(UUID characterId) {
        // IdleLayer benötigt kein spezifisches Reset
    }
    ...
    private Rotation interpolateRotation(Rotation start, Rotation end, float factor) {
        ...
        return new Rotation(
                start.getPitch() + (end.getPitch() - start.getPitch())
                        * dampedFactor,
