...
public class AdvancedWalkingLayer implements IMotionLayer {
    ...
    private float getStepLength(GaitType gaitType) {
        switch (gaitType) {
            case SNEAKING: return SNEAKING_STEP_LENGTH;
            case LIMPING: return LIMPING_STEP_LENGTH;
            default: return NORMAL_STEP_LENGTH;
        }
    }
    
    private float getStepHeight(GaitType gaitType) {
        switch (gaitType) {
            case SNEAKING: return SNEAKING_STEP_HEIGHT;
            case LIMPING: return LIMPING_STEP_HEIGHT;
            default: return NORMAL_STEP_HEIGHT;
        }
    }
    ...
    private float calculateVerticalOffset(float cycle, float stepHeight, GaitType gaitType) {
        switch (gaitType) {
            case LIMPING:
                ...
                return stepHeight * (float)Math.max(0, Math.sin(cycle) * (1 + 0.5f * Math.sin(cycle/2)));
            case SNEAKING:
    ...
    public MotionState processPhysics(UUID characterId, PhysicsData physicsData) {
        ...
        Position adjustedPosition = new Position(
            ...
            Math.max(0, physicsData.getPosition().getY() - 9.81f * physicsData.getDeltaTime() * verticalAdjustment),
